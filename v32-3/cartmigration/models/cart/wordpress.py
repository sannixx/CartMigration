...
class LeCartWordpress(LeBasecart):
    ...
    def clear_target_blogs(self):
        ...
        all_queries = {
            ...
            'term': {
                'type': 'delete',
                'query': 'DELETE FROM _DBPRF_terms WHERE term_id IN (SELECT term_id FROM _DBPRF_term_taxonomy WHERE taxonomy IN ' + self.list_to_in_condition(
                    ['category', 'post_tag']) + ')'
            },
            'term_taxonomy': {
                'type': 'delete',
                'query': 'DELETE FROM _DBPRF_term_taxonomy WHERE taxonomy IN ' + self.list_to_in_condition(
                    ['category', 'post_tag'])
            },
            'term_relationship': {
                'type': 'delete',
    ...
    def get_blogs_main_export(self):
        ...
        query = {
            ...
            'query': "SELECT * FROM _DBPRF_posts WHERE ID > " + to_str(
                id_src) + "  AND post_type = 'post' ORDER BY ID ASC LIMIT " + to_str(limit),
        }
    ...
    def convert_blog_export(self, blog, blogs_ext):
        ...
        blog_data['title'] = blog['post_title'] if blog['post_title'] else (
            blog['post_name'] if blog['post_name'] else 'blog-' + to_str(blog['ID']))
        ...
        if thumbnail_id:
            ...
            if thumbnail_src and thumbnail_src[0].get('meta_value'):
                ...
                blog_data['thumb_image']['url'] = self._notice['src']['cart_url'].rstrip(
    ...
    def blog_import(self, convert, blog, blogs_ext):
        blog_data = {
            ...
            'post_excerpt': self.replace_url_src_to_url_target(
                self.change_img_src_in_text(get_value_by_key_in_dict(convert, 'short_content', ''), True)),
    ...
    def after_blog_import(self, blog_id, convert, blog, blogs_ext):
        ...
        all_queries.append(self.create_update_query_connector('posts', {
            'guid': self._notice['target']['cart_url'] + "/?p=" + to_str(blog_id)}, {'ID': blog_id}))
        ...
        if tags_src:
            check_exist_term_query = {
                ...
                'post_tag': {
                    'type': 'select',
                    'query': "SELECT * FROM _DBPRF_terms AS t JOIN _DBPRF_term_taxonomy As tt ON t.term_id = tt.term_id WHERE tt.taxonomy = 'post_tag' AND t.name IN " + self.list_to_in_condition(
                        tags_src)
                }
            }
        ...
        if convert.get('review'):
            ...
            for review in convert['review']:
                ...
                review_id = self.import_review_data_connector(
                    self.create_insert_query_connector('comments', review_data), 'review')
                if review_id:
    ...
    def convert_page_export(self, page, pages_ext):
        ...
        page_data['title'] = page['post_title'] if page['post_title'] else (
            page['post_name'] if page['post_name'] else 'blog-' + to_str(page['ID']))
    ...
    def page_import(self, convert, page, pages_ext):
        ...
        while check_slug_exist:
            check_slug_exist = True if self.select_map(self._migration_id, self.TYPE_PAGE, None, None, None,
                                                       code_name) else False
            if check_slug_exist:
    ...
    def after_page_import(self, page_id, convert, page, pages_ext):
        ...
        if self._notice['target']['support']['yoast_seo']:
            page_meta = {
                '_yoast_wpseo_title': to_str(convert['meta_title']).replace('%', '') if 'meta_title' in convert and
                                                                                        convert['meta_title'] else '',
                '_yoast_wpseo_metadesc': to_str(convert['meta_description']).replace('%',
        ...
        if convert['images']:
            for image in convert['images']:
                ...
                for meta_key, value in postmeta.items():
                    ...
                    self.import_page_data_connector(self.create_insert_query_connector('postmeta', postmeta_data), True,
                                                    convert['id'])
        data_revision = {
            'post_author': 1,
            'post_date': convert['created_at'] if convert['created_at'] and '0000-00-00' not in convert[
    ...
    def wp_image(self, path, image_details=None, label='', convert=dict):
        url = self._notice['target']['cart_url'].rstrip('/') + '/' + self._notice['target']['config']['image'].rstrip(
            '/') + '/' + path.lstrip('/')
        ...
        post_data = {
            'post_author': 1,
            'post_date': convert['created_at'] if isinstance(convert, dict) and convert.get(
                'created_at') else get_current_time(),
            ...
            'to_ping': '',
            'pinged': '',
            'post_modified': convert['updated_at'] if isinstance(convert, dict) and convert.get(
    ...
    def get_image_size(self):
        ...
        res = self.select_data_connector(
            self.create_select_query_connector('options', {'option_name': 'woocommerce_single_image_width'}))
    ...
    def install_module_connector(self, request):
        ...
        if not self._cart_url or not request.get(key_api) or not request[key_api].get('account') or not request[
            key_api].get('password'):
            return response_error('Info Invalid. Please enter the correct url, username and password',
                                  elm_module_connector, 'Info Invalid')
        ...
        wp_install_connector = wp_admin + "/admin.php?page=install-connector&token=" + (
            request.get('src_token') if self._type == 'src' else request.get('target_token'))
        ...
        with requests.Session() as wp_session:
            try:
                if self._notice[self.get_type()]['config'].get('auth'):
                    wp_session.auth = (self._notice[self.get_type()]['config']['auth'].get('user'),
                                       self._notice[self.get_type()]['config']['auth'].get('pass'))
                headers = {
                ...
                if login_page.status_code == 404:
                    response = response_error(
                        'We can not access {}, Please <a href="#" hide="#{}-module-connector-auto" show="#{}-module-connector-manually" class="js-btn-module-connector">click</a> to manually upload connector!'.format(
                            url_to_link(wp_login), self._type, self._type), elm_cart_url, 'Url invalid')
                ...
                if login.status_code not in [200, 302]:
                    login = wp_session.post(post_login_url, headers=headers, data=data_login, allow_redirects=False,
                                            proxies=self.convert_proxy_request(self.PROXY_HOST))
                ...
                if login_error:
                    return response_error(
                        'The account sign-in was incorrect or your account is disabled temporarily. Please enter correct account wordpress admin!',
                        elm_module_connector, 'Account incorrect')
                ...
                plugin_connector = plugin_install_content.find("div", {
                    "class": "plugin-card-litextension-data-migration-to-woocommerce"})
                ...
                if action_install:
                    ...
                    if install_content.find("div", {'id': 'request-filesystem-credentials-form'}):
                        return response_error(
                            'Failed to automatically install the plugin. Please disable FTP Credentials request following the steps from <a href = "https://litextension.com/faq/docs/userguide-demo/connector/i-keep-getting-ftp-access-required-message/" target = "_blank">this Guide</a> or <a href="#" hide="#{}-module-connector-auto" show="#{}-module-connector-manually" type_upload = "connector" class="js-btn-module-connector">click</a> to manually upload connector!'.format(
                                self._type, self._type), elm_cart_url, 'FTP access required.')

                    link_active = install_content.find_all('a', {'target': '_parent', 'class': 'button-primary'})
                    if not link_active:
                        return response_error(
                            'Failed to automatically install the plugin. Please <a href="#" hide="#{}-module-connector-auto" show="#{}-module-connector-manually" class="js-btn-module-connector">click</a> to manually upload connector!'.format(
                                self._type, self._type), elm_cart_url, 'Install failed.')
                ...
                if install_connector_content.find('body', {'id': "error-page"}):
                    return response_error(
                        'Failed to active. Please login to wordpress admin, active plugin <b>LitExtension: Migrate Shopping Carts to WooCommerce</b>. After active, please <a href="{}" target="_blank">click</a> to install Connector and try again'.format(
                            wp_install_connector), elm_cart_url, 'Active failed.')
                response = install_connector_content.find('p', {'id': "litextension-response"})
                if not response:
                    return response_error(
                        'Failed to automatically install the connector. Please <a href="#" hide="#{}-module-connector-auto" show="#{}-module-connector-manually" class="js-btn-module-connector">click</a> to manually upload connector!'.format(
                            self._type, self._type), elm_cart_url, 'Install failed.')
    ...
    def get_link_change_token(self):
        wp_url = self.format_url(self._cart_url)
        return wp_url + "/wp-admin/admin.php?page=install-connector&token=" + self._notice[self._type]['config'][
            'token']
    ...
    def detect_root_url(self):
        ...
        while True:
            ...
            try:
                exist = requests.head(to_str(url).rstrip('/') + "/wp-login.php",
                                      headers={"User-Agent": self.get_random_useragent(),
                                               "Referer": 'https://google.com'}, timeout=10, verify=False)
    ...
    def error_root_url(self, root_url):
        if root_url != self._cart_url:
            return 'We can not access {}, but we detect {} is accessible, if {} is your root url please use it as Url'.format(
                url_to_link(self._cart_url + "/wp-login.php"), url_to_link(root_url + "/wp-login.php"),
                url_to_link(root_url))
        return ''
