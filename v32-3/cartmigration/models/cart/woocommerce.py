...
class LeCartWoocommerce(LeCartWordpress):
    ...
    def clear_target_products(self):
        ...
        while self._check_product_exists():
            ...
            clear_table = self.get_connector_data(self.get_connector_url('query'),
                                                  {'query': json.dumps(del_term_rela)})
            ...
            all_images_query = {
                'type': 'query',
                'query': "Delete FROM wp_posts WHERE post_title = 'images' AND post_type = 'attachment'"
            }
            clear_table = self.get_connector_data(self.get_connector_url('query'),
                                                  {'query': json.dumps(all_images_query)})
            if (not clear_table) or (clear_table['result'] != 'success') or (not clear_table['data']):
                self.log("Clear data failed. Error: Could not empty products", 'clear')
                continue
    ...
    def clear_target_customers(self):
        ...
        try:
            ...
            # print(del_customer_lookup_query)
            clear_table = self.get_connector_data(self.get_connector_url('query'), {
                'query': json.dumps({
                    'type': 'query', 'query': del_customer_lookup_query
                })
    ...
    def clear_target_orders(self):
        ...
        while self._check_order_exists():
            ...
            # phong dh begin test block
            all_order_meta_query = {
                'type': 'select',
                'query': ''' DELETE wp_woocommerce_order_items,wp_woocommerce_order_itemmeta FROM wp_woocommerce_order_items
inner join wp_woocommerce_order_itemmeta on wp_woocommerce_order_items.order_item_id = wp_woocommerce_order_itemmeta.order_item_id'''
            ...
            clear_table = self.get_connector_data(self.get_connector_url('query'),
                                                  {'query': json.dumps(all_order_meta_query)})
    ...
    def category_import(self, convert, category, categories_ext):
        ...
        category_term_query = self.create_insert_query_connector('wp_terms', category_term)
        ...
        category_taxonomy_query = self.create_insert_query_connector('wp_term_taxonomy', category_taxonomy)
    ...
    def product_import(self, convert, product, products_ext):
        ...
        product_query = self.create_insert_query_connector('wp_posts', product_posts_data)
        ...
        # product_import = self.import_product_data_connector(
        #     self.create_insert_query_connector('wp_posts', product_data), True,
        #     convert['id'])
    ...
    def after_product_import(self, product_id, convert, product, products_ext):
        ...
        if convert['thumb_image']['url'] or convert['thumb_image']['path']:
            image_process = self.process_image_before_import(convert['thumb_image']['url'],
                                                             convert['thumb_image']['path'])
            # image_process = {'url': 'http://0.0.0.0:8085/customcart/images/cysmmbarvantoffcrnch8ct-s.png',
            # 'path': 'cysmmbarvantoffcrnch8cts.png'}
            image_import_path = self.uploadImageConnector(image_process, self.add_prefix_path(
        ...
        # wp-content/uploads/2020/12/cysmmbarvantoffcrnch8cts_4nd.png
        ...
        for meta_key, value in product_metadata.items():
            ...
            all_query.append(self.create_insert_query_connector('wp_postmeta', postmeta_data))
        ...
        query_delete = {
            'type': 'delete',
            'query': 'DELETE FROM wp_term_relationships WHERE object_id = ' + to_str(
                product_id) + ' AND term_taxonomy_id IN (SELECT term_taxonomy_id FROM wp_term_taxonomy WHERE taxonomy IN ' + self.list_to_in_condition(
                ['product_brand', 'product_cat']) + ')'
        }
        all_queries.append(query_delete)
        # category
        all_categories = list()
        if convert['categories']:
            for category in convert['categories']: #{'id': '14', 'code': None, 'position': 0}
            ...
            for cate_id in all_categories:
                ...
                category_query = self.create_insert_query_connector("wp_term_relationships", category_data)
                all_queries.append(category_query)
        self.import_multiple_data_connector(all_queries, 'update_product')
    ...
    def customer_import(self, convert, customer, customers_ext):
        ...
        users_metadata = {
            ...
            'user_registered': convert['customers_info_date_account_created'] if convert['customers_info_date_account_created'] else '',
            'display_name' : f"{convert['first_name']} {convert['last_name']}",
            'user_activation_key' : ''
        }
        customer_queries = self.create_insert_query_connector('wp_users', users_metadata)
        ...
        users_meta = {
            ...
            'wp_user_level' : 0,
            'last_update': convert['customers_info_date_account_last_modified'] if convert['customers_info_date_account_last_modified'] else get_current_time(),
            'billing_address_1': '',
            'billing_company': '',
            'billing_last_name': '',
            'billing_first_name': '',
        ...
        for meta_key, value in users_meta.items():
            ...
            all_query.append(self.create_insert_query_connector('wp_usermeta', usermeta_data))
        ...
        customer_data_lookup = {
            ...
            'last_name': convert['last_name'],
            'email': convert['email'],
            # 'country': convert['address'],
            'date_last_active': convert['customers_info_date_of_last_logon'],
            'date_registered': convert['customers_info_date_account_created'] if convert['customers_info_date_account_created'] else '',
        ...
        customer_data_lookup_queries = self.create_insert_query_connector('wp_wc_customer_lookup', customer_data_lookup)
        self.import_data_connector(customer_data_lookup_queries, 'customer_metadata')
        #end of test

        # customer_meta_queries = self.create_insert_query_connector('wp_users', users_metadata)
        # customer_id = self.import_data_connector(customer_queries, 'ID')

        self.insert_map(self.TYPE_CUSTOMER, convert['id'], customer_id, convert['email'], users_metadata['user_email'])

        # test = self.get_map_field_by_src(map_type=self.TYPE_CUSTOMER, id_src=convert['id'],field='id_desc')
        # print(test)
    ...
    def order_import(self, convert, order, orders_ext):
        ...
        order_query = self.create_insert_query_connector('wp_posts', order_dataset)
    ...
    def after_order_import(self, order_id, convert, order, orders_ext):
        ...
        for item in convert['items']:
            # print(self.get_map_field_by_src(map_type=self.TYPE_PRODUCT, id_src=item['id'], field='value')['name'])
            ...
            item_insert_query = self.create_insert_query_connector('wp_woocommerce_order_items', meta_data)
            ...
            for meta_key, value in item_metadata.items():
                ...
                item_metadata_all_query.append(self.create_insert_query_connector('wp_woocommerce_order_itemmeta', item_meta_data))

            self.import_multiple_data_connector(item_metadata_all_query, 'item_metadata', True)

        # phongdh
        # insert ship service link to oder by order_id
        meta_data_ship = {
            'order_item_name': 'Shipping',
            'order_item_type': 'shipping',
            'order_id': order_id
        }
        meta_data_ship_query = self.create_insert_query_connector('wp_woocommerce_order_items', meta_data_ship)
        ship_id = self.import_data_connector(meta_data_ship_query)

        shipping_metadata = {
            'method_id' : '',
            'instance_id' : 0,
            'cost' : float(convert['shipping']['amount']) if convert['shipping']['amount'] else 0,
            'total_tax': float(convert['tax']['amount']) if convert['tax']['amount'] else 0,
            'taxes' : "a:1:{s:5:\"total\";a:1:{i:1;s:1:\""+to_str(int(convert['tax']['amount']) if convert['tax']['amount'] else 0) +"\";}}"
        }

        ship_metadata_all_query = []
        for meta_key, value in shipping_metadata.items():
            ship_item_meta_data = {
                'order_item_id': ship_id,
                'meta_key': meta_key,
                'meta_value': value
            }
            ship_metadata_all_query.append(self.create_insert_query_connector('wp_woocommerce_order_itemmeta',ship_item_meta_data))

        self.import_multiple_data_connector(ship_metadata_all_query, 'ship_item_metadata', True)
        ...
        #
        meta_data_tax = {
            'order_item_name': 'EX-TAX-1',
            'order_item_type': 'tax',
            'order_id': order_id
        }
        meta_data_tax_query = self.create_insert_query_connector('wp_woocommerce_order_items', meta_data_tax)
        tax_id = self.import_data_connector(meta_data_tax_query)
        #
        ...
        tax_metadata = {
            ...
            'compound' : "",
            'tax_amount' : 0,
            'shipping_tax_amount' : float(convert['tax']['amount']) if float(convert['tax']['amount']) else 0,
            'rate_percent' : 0,
        }
        #
        tax_metadata_all_query = []
        for meta_key, value in tax_metadata.items():
            tax_item_meta_data = {
                'order_item_id': tax_id,
                'meta_key': meta_key,
                'meta_value': value
            }

            tax_metadata_all_query.append(
                self.create_insert_query_connector('wp_woocommerce_order_itemmeta', tax_item_meta_data))

        self.import_multiple_data_connector(tax_metadata_all_query, 'tax_item_metadata', True)
